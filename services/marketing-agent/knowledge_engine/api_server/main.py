"""
knowledge_engine/api_server/main.py

启动: python -m api_server.main
...
def load():
    ...
    _data["basic"] = pd.read_csv(os.path.join(s, "customer_basic.csv"))
    ...
    # 合并客户基本信息中的name字段到snapshot
    _data["snapshot"] = _data["snapshot"].merge(
        _data["basic"][["cust_id", "name", "gender", "occupation", "education", "phone"]],
        on="cust_id", how="left"
    )
    # 加载频控规则
    ...
    for _, row in page_df.iterrows():
        ...
        customers.append({
            ...
            "profile": {
                ...
                "occupation": row.get("occupation", "") if pd.notna(row.get("occupation")) else "",
                "income_level": row["income_level"], "lifecycle_stage": row["lifecycle_stage"],
                "vip_tier": row["vip_tier"],
                "education": row.get("education", "") if pd.notna(row.get("education")) else "",
...
from profile_engine.router_feedback import router as feedback_router
