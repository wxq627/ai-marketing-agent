...
class ASRTemplates:
    ...
    def cross_border_01() -> tuple:
        ...
        return (
            [
                ...
                {"role": "agent", "text": "您好！去日本旅游用招行信用卡非常方便。首先建议您开通境外免货币转换费服务，这样日元消费直接按银联汇率结算，不额外收费。另外您可以看看您的卡是否有境外返现权益。"},
                {"role": "customer", "text": "我的是白金卡，有境外返现吗？"},
                {"role": "agent", "text": "有的！白金卡境外消费返现最高5%，月上限500元。而且日本很多百货商场（如三越、伊势丹）刷招行卡还有额外折扣。"},
    ...
    def benefits_01() -> tuple:
        ...
        return (
            [
                ...
                {"role": "agent", "text": "招行积分可以兑换南航明珠里程，兑换比例大约是25积分换1里程。您的8万积分大概可以换3200南航里程，足够兑换一张短途机票或者升舱。"},
    ...
    def dormant_01() -> tuple:
        ...
        return (
            [
                {"role": "customer", "text": "你好，我有一张招行信用卡很久没用了，想确认一下还能不能用。"},
    ...
    def new_user_01() -> tuple:
        ...
        return (
            [
                {"role": "customer", "text": "你好，我刚申请了招行信用卡，想问下开卡之后有什么福利？"},
                {"role": "agent", "text": "恭喜您成为招行信用卡持卡人！目前新户有三重礼：第一，开卡后30天内任意消费1笔送100元刷卡金；第二，首笔消费满99元可以选招行定制礼品（保温杯/蓝牙耳机/U盘三选一）；第三，首次绑定支付宝、微信和云闪付各送10元红包。"},
    ...
    def general_01() -> tuple:
        ...
        return (
            [
                ...
                {"role": "agent", "text": "您好，更新手机号可以在掌上生活APP上操作，路径是：我的→设置→安全中心→修改手机号。需要验证身份证号和原手机号接收验证码。如果您原手机号已经不能用了，需要携带身份证到任意招行网点办理。"},
